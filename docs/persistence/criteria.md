...
```json
{
  ...
  "grouping":      {"by": [{"field": "author_id"}, {"field": "printed_at", "grain": "month"}],
                    "totals": {"pages": ["sum", "pages"]}, "depth": 2},
  "count":         "capped",
  "meta":          true
}
```
...
| `grouping` | how to split the set when counting rather than listing | one statement per level, buckets with counts and folds; `depth` says how many levels now |
