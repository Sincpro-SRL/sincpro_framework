...
class MkDocsMarkdownGenerator:
    ...
    def _generate_class_docs(self, cls: ClassMetadata) -> List[str]:
        ...
        if cls.docstring:
            processed_docstring = self._process_docstring(cls.docstring, cls.name)
        ...
        if cls.methods:
            ...
            for method_name, method in cls.methods.items():
                ...
                if method.docstring:
                    processed_method_docstring = self._process_docstring(method.docstring, f"{cls.name}.{method_name}")
    ...
    def _process_docstring(self, docstring: str, component_name: str) -> str:
        """
        ...
        for line in lines:
            ...
            if ("Example:" in line or 
                "@framework." in line or 
                "class My" in line or
                "def execute(self, dto:" in line or
                line.strip().startswith("@") or
                "external_service:" in line or
                "correlation_id =" in line or
                "return MyResponseDTO" in line):
            ...
            if line_stripped.startswith('- ') and any(pattern.lower() in line.lower() for pattern in [
                "non-atomic", "coordinating", "complex business", "aggregating data", "multiple steps"
            ]):
                continue
        ...
        if not result or len(result) < 10:
            ...
            for line in first_lines:
                ...
                if line and not any(pattern in line for pattern in framework_patterns):
                    ...
                    if not ("ApplicationService" in line and ("coordinate" in line or "Features" in line)):
