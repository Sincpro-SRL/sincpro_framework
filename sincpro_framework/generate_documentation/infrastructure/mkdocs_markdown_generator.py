...
class MkDocsMarkdownGenerator:
    ...
    def _generate_dependencies_page(self) -> str:
        ...
        if functions:
            lines.extend(["## Dependency Functions", ""])
            for func in functions:
                lines.extend(self._generate_function_docs(func))
                lines.append("")

        if classes:
            lines.extend(["## Dependency Classes", ""])
            for cls in classes:
    ...
    def _generate_middlewares_page(self) -> str:
        ...
        if functions:
            lines.extend(["## Middleware Functions", ""])
            for func in functions:
                lines.extend(self._generate_function_docs(func))
                lines.append("")

        if classes:
            lines.extend(["## Middleware Classes", ""])
            for cls in classes:
                lines.extend(self._generate_class_docs(cls))
                lines.append("")
    ...
    def _generate_function_docs(self, func: FunctionMetadata) -> List[str]:
        ...
        if func.return_type and func.return_type != "Any":
            lines.extend([f"**Returns:** `{func.return_type}`", ""])
