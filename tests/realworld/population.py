...
entries. Entries are written in batches inside one `context()` each, committed per batch: the
shape a real import has, and the one that keeps memory flat at the stress volume.
...
def populate(repository: Repository, entries: int, seed: int = SEED) -> Census:
    ...
    with repository.context() as ledger:
        ledger.session.add_all(journals + partners)
        ledger.commit()
    ...
    for start in range(0, entries, BATCH):
        ...
        with repository.context() as ledger:
            ledger.session.add_all(batch_entries)
            ledger.session.add_all(batch_lines)
            ledger.commit()

    for account in accounts:
        account.balance = balances[account.id]
    with repository.context() as ledger:
        ledger.session.add_all(accounts)
        ledger.commit()
